...
import { Card, Button, Badge, Empty, Spinner, Input, Select } from '../../components/ui';
...
export default function ApplicationHistory() {
  ...
  const loadApps = useCallback(async () => {
    ...
    try {
      ...
      const [appsRes, statsRes] = await Promise.all([
        api.get(`/job-applications/history?${params.toString()}`),
  ...
  const cleanup = async () => {
    if (!confirm('Delete applications older than 30 days? This cannot be undone.')) return;
    setCleaning(true);
    try {
      const res = await api.post('/job-applications/cleanup?max_age_days=30');
      alert(`Deleted ${res.data.deleted} old applications`);
      loadApps();
    } catch (e) {
    ...
    <Layout title="Application History">
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12, marginBottom: 24 }}>
          ...
          {(Object.entries(stats.by_status || {})).map(([status, count]) => (
            <Card key={status} style={{ padding: 16, textAlign: 'center' }}>
      ...
      <div className="flex" style={{ gap: 8, marginBottom: 16 }}>
        ...
        <Select
          ...
          style={{ flex: 1 }}
        />
        <Button variant="outline" onClick={cleanup} disabled={cleaning} style={{ flexShrink: 0 }}>
              ...
              <div className="flex" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="flex" style={{ gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    ...
                    <Badge color={STATUS_COLORS[app.status] || 'var(--muted)'} style={{ fontSize: 10 }}>
                      {app.status.replace('_', ' ')}
                    </Badge>
                  </div>
                  <div className="muted" style={{ fontSize: 13 }}>
                    {app.company_name || 'Unknown Company'}
                    {app.ats_platform && app.ats_platform !== 'unknown' && (
                      <span style={{ marginLeft: 8, opacity: 0.6 }}>({app.ats_platform})</span>
                    )}
                  </div>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 16 }}>
                  ...
                  {app.total_fields > 0 && (
                    <div className="muted" style={{ fontSize: 11, marginTop: 4 }}>
                      {app.auto_filled}/{app.total_fields} fields filled
                    </div>
                  )}
                  {app.match_score && (
                    <div style={{ fontSize: 12, marginTop: 4, color: app.match_score >= 0.8 ? 'var(--success)' : 'var(--muted)' }}>
